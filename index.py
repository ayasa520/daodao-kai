...
def change_data_handle(number, data, type, search_time_limit, search_time_limit_num, zone, now_time, user_info, since,
    ...
    if type == "combine":
        ...
        if len(list) < number:
            ...
            change_data(user_info, handle_id,
                        '{"content":"' + handle_data + '",\n"user_agents":"' + str(user_agent) + '"}')
        ...
        if 'k' in parse.parse_qs(o.query):
            ...
            if data == os.environ["DAODAO_PASSWORD"]:
                if 'g' in parse.parse_qs(o.query):
                    ...
                    text = change_data_handle(int(data), '', 'combine', search_time_limit, search_time_limit_num, zone,
                                              now_time, user_info, since, user_agent)
                elif 'a' in parse.parse_qs(o.query):
                    data = parse.parse_qs(o.query)['a'][0]
                    data = data.split(',', 1)
                    text = change_data_handle(int(data[0]), data[1], 'append', search_time_limit, search_time_limit_num,
                                              zone, now_time, user_info, since, user_agent)
                elif 'e' in parse.parse_qs(o.query):
                    data = parse.parse_qs(o.query)['e'][0]
                    data = data.split(',', 1)
                    text = change_data_handle(int(data[0]), data[1], 'edit', search_time_limit, search_time_limit_num,
                                              zone, now_time, user_info, since, user_agent)
                elif 'c' in parse.parse_qs(o.query):
                    data = parse.parse_qs(o.query)['c'][0]
                    text = creat_data(now_time, user_info,
